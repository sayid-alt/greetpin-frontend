"use client"
import { Plus } from "lucide-react";
import ButtonSchedule from "./ButtonSchedule";

export default function FloatingActionButton() {
  return (
    <>
      <ButtonSchedule 
        style={`fixed bottom-8 right-8 w-10 h-10 bg-[#adc6ff] text-[#002e6a] rounded-full shadow-2xl flex items-center justify-center hover:scale-110 active:scale-95 transition-all z-50 group`}
      >
          <Plus className="group-hover:rotate-90 transition-transform duration-300" size={25} />
          <span className="absolute right-16 bg-[#2e3545] px-3 py-2 border border-[#424754] rounded text-xs text-[#dce2f7] font-semibold opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity whitespace-nowrap shadow-md">
            Quick Schedule
          </span>
      </ButtonSchedule>
    </>
      
  );
}