import { ApiDataWrapper, IMPORTANCE_LEVEL_CONFIG, ImportanceLevel } from '@/app/config/componentConfig';
...
const NextEventCard = ({ response }: { response: ApiDataWrapper }) => {
    const nextEvent = response.data[0];

    let currentLevel = nextEvent.importanceLevel || "LOW";
    ...
    return (
        <>
            <div className="bg-[#191f2f]/70 backdrop-blur-xl p-6 rounded-xl flex flex-col border border-[#424754]/50 min-h-[160px]">
                ...
                {/* Event Details */}
                <div className="mb-4">
                    <p className="text-xs text-[#c2c6d6] mb-1">Starts {moment(nextEvent.startDateTime).fromNow()}</p>
